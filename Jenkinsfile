node('agent') {
  ...
  stage('SonarQube analysis') {
    ...
    withSonarQubeEnv('sonarqube') {
      sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectName=curl -Dsonar.projectVersion=1.0 -Dsonar.projectKey=curl:app -Dsonar.scm.provider=git -Dsonar.sources=./src -Dsonar.cxx.includeDirectories=./include/*"	 
    }
  }
  stage('Build') {
  sh "./buildconf"
